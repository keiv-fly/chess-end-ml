...
m = Sequential()
m.add(Conv2D(8, (1, 1), activation='relu', input_shape=(8,8,8), padding="same"))
m.add(Conv2D(4, (1, 1), activation='relu', padding="same"))
...
hist2 = m.fit(X, y, batch_size=32, epochs=4)
y_train = m.predict(X, batch_size=1024*8, verbose=1)
y_train_cat=(y_train>0.5)*2
y_train_cat=y_train_cat.flatten()
acc = np.sum(y_train_cat==df["wdl"])/y_train_cat.shape[0]
