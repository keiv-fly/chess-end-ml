...
X = np.load("data/KPvK_w_dist.npy")
X = np.swapaxes(X,1,2)
X = np.swapaxes(X,2,3)
...
y = (df["wdl"]==2)*1
m = Sequential()
m.add(Conv2D(6, (5, 5), activation='relu', input_shape=(8,8,5), padding="same"))
...
hist = m.fit(X, y, batch_size=1024*8, epochs=20)
...
hist2 = m.fit(X, y, batch_size=1024*8, epochs=20)
y_train = m.predict(X, batch_size=1024*8, verbose=1)
